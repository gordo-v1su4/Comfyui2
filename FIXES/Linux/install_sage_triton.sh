...
echo
echo "V4.3 Automatic Nightly 2.7/2.8 PyTorch, Triton and Sage 2 installation script for Linux"
echo "This script will install PyTorch 2.7 (Nightly) with CUDA 12.4 or PyTorch 2.8 (Nightly) with CUDA 12.6 or 12.8."
echo "NB: If you want to use FastFP16 (extra ~10 percent), you will need CUDA 12.6 or 12.8 installed."
echo "NB: Sage2 will work on its own with PyTorch 2.7 or 2.8 with CUDAs 12.4, 12.6 or 12.8."
echo
...
# Ask for CUDA version to use
echo
echo "Choose CUDA version to use for PyTorch installation:"
echo "[1] CUDA 12.4"
echo "[2] CUDA 12.6"
echo "[3] CUDA 12.8 (recommended if your driver supports it)"
read -p "Enter your choice (1/2/3): " CUDA_CHOICE
...
if [ "$CUDA_CHOICE" = "1" ]; then
    ...
    CUDA_VERSION="12.6"
    CLEAN_CUDA="126"
elif [ "$CUDA_CHOICE" = "3" ]; then
    CUDA_VERSION="12.8"
    CLEAN_CUDA="128"
else
    echo "Invalid choice. Using CUDA 12.6 as default."
    CUDA_VERSION="12.6"
    CLEAN_CUDA="126"
fi
...
echo "Installing PyTorch nightly with CUDA $CUDA_VERSION..."
pip install --pre torch torchvision torchaudio --index-url https://download.pytorch.org/whl/nightly/cu$CLEAN_CUDA
...
git pull
cd ..
pip install --upgrade --pre torch torchvision torchaudio --extra-index-url https://download.pytorch.org/whl/nightly/cu$CLEAN_CUDA -r ComfyUI/requirements.txt pygit2
EOF
...
cd /root/ComfyUI-Easy-Install/ComfyUI-Easy-Install
source venv/bin/activate
python ComfyUI/main.py --use-sage-attention --fast fp16_accumulation
EOF
